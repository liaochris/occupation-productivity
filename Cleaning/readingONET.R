...
# function for reading in files from ONET for each year
...
occupation_data <- rbind(
  read_files("Occupation Data.txt", ctypes, start),
...
occupation_metadata <- rbind(
  read_files("Occupation Level Metadata.txt", ctypes, start),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 7, "0/", "OccLevelMetadata.txt", sep = "")),
    col_types = ctypes, delim = "\t"
  ),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 9, "0/", "Occupation Level Metadata.txt", sep = "")),
    col_types = ctypes, delim = "\t"
  )
)
occupation_metadata_temp <- read_delim(
  file = paste(paste("Datasets/Imported/ONET Data/db_", 5, "1/", "OccLevelMetadata.txt", sep = "")),
  col_types = ctypes, delim = "\t"
)
...
content_model <- rbind(
  read_files("Content Model Reference.txt", ctypes, start),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 5, "1/", "onet_content_model_reference.txt", sep = "")),
    delim = "\t"
  ),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 7, "0/", "onet_content_model_reference.txt", sep = "")),
    delim = "\t"
  ),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 9, "0/", "Content Model Reference.txt", sep = "")),
    delim = "\t"
  )
) %>%
  distinct() %>%
...
scales_reference <- rbind(
  read_files("Scales Reference.txt", ctypes, start),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 9, "0/", "Scales Reference.txt", sep = "")),
    delim = "\t"
  ),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 7, "0/", "scales_reference.txt", sep = "")),
    delim = "\t"
  ),
  read_delim(
    file = paste(paste("Datasets/Imported/ONET Data/db_", 5, "1/", "scales_reference.txt", sep = "")),
    delim = "\t"
  )
) %>%
  distinct() %>%
...
tasks_old <- rbind(
  read_delim(file = paste(paste("Datasets/Imported/ONET Data/db_", 5, "1/", "Tasks.txt", sep = "")), delim = "\t"),
...
colnames(tasks_old_wide_RT) <- str_replace(
  str_replace(colnames(tasks_old_wide_RT), "Percent Frequency: ", ""),
  "-F[1-7]", ""
)
...
task_rat_RT_wide <- task_rat_RT_wide %>%
  filter(!is.na(task_rat_RT_wide$`Task ID`)) %>%
  distinct()


# work activity import
start <- 11
ctypes <- paste(c(rep("c", 4), rep("n", 5), rep("c", 4)), collapse = "")
work_act <- rbind(
  read_files("Work Activities.txt", ctypes, start),
...
occ_abilities_work <- inner_join(abilities_to_work,
  abilities %>%
    ...
    rename(
      `Occupation Ability Score` = `Data Value`,
  ...
  by = c(
    "Abilities Element ID" = "Element ID",
...
work_DWA <- inner_join(work_act %>%
  dplyr::select(-c(`Standard Error`, `Lower CI Bound`, `Upper CI Bound`)) %>%
  rename(
    `Work Activities Element ID` = `Element ID`,
  ...
  ), DWA_ref, by = c("Work Activities Element ID" = "Element ID"))
...
task_work_abilities <- inner_join(task_work,
  occ_abilities_work,
  by = c("O*NET-SOC Code", "Work Activities Element ID", "Work Activities Element Name", "Work Activities Date" = "Ability Date")
) %>%
  rename("Date" = "Work Activities Date") %>%
  distinct()

# adding task rating data to the dataset
task_rat_work_abilities <- inner_join(
  task_work_abilities,
  task_rat_RT_wide %>%
    rename(
      "Task Ratings Scale ID" = "Scale ID",
...
matching_dataset_abilities <- task_rat_work_abilities %>%
  dplyr::select(-c(
    "Work Activities N", "Work Activities Recommend Suppress", "Work Activities Not Relevant",
...
occ_skills_work <- inner_join(skills_to_work,
  ...
  by = c("Skills Element ID" = "Element ID", "Skills Element Name" = "Element Name")
)
...
# adding task rating data to the dataset
task_rat_work_skills <- inner_join(
  task_work_skills,
  task_rat_RT_wide %>%
    rename(
      "Task Ratings Scale ID" = "Scale ID",
